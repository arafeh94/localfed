...
for round_id in range(10):
    for client_id, data in clients_data.items():
        ...
        tools.train(client_model, data.map(lambda x, y: (x, 0)).batch(batch_size), epochs=1000, lr=0.0001)
...
for cid, model in clients_model.items():
    ...
    tt = tools.infer(model, test_data[cid].map(lambda x, y: (x, 0)).batch(batch_size))
    # tt = tools.infer_detailed(model, test_data[cid].map(lambda x, y: (x, 0)).batch(125))
    final_results.append(tt[0])
    matrix[cid][cid] = tt[0]
    ft = tools.infer(model, test_data[cid].map(lambda x, y: (x, 1)).batch(batch_size))
    ...
    for i in range(max_clients):
        ...
        tf = tools.infer(model, test_data[i].map(lambda x, y: (x, 0)).batch(batch_size))
        results_TF.append(tf[0])
        # print(cid, False, tf)

        ff = tools.infer(model, test_data[i].map(lambda x, y: (x, 1)).batch(batch_size))
        matrix[cid][i] = tf[0]
...
heatmap(matrix)
# print(tools.infer(clients_model[60], test_data[60].map(lambda x, y: (x, 0)).batch(batch_size)))
